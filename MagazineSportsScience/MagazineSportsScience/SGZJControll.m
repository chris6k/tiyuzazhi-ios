...
#import "SGZJControll.h"

@implementation SGZJControll
...
- (IBAction)DoFilterProcess:(id)sender {
    ...
    if([self IsLogining])
    {
        ...
        if(![self IsLoading])
        {
            [self  performSegueWithIdentifier:@"segueFilterProcessFromSGZJ" sender:self];
        }
    }
...
- (IBAction)unwindSGZJViewController:(UIStoryboardSegue *)segue
{
         ...
         cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView == TableExamArts)
    {
        ...
        if ([[ AllexamArts.data objectAtIndex:indexPath.row]   comment ]!=nil) {
            cell.lb_summary.text = [[ AllexamArts.data objectAtIndex:indexPath.row]   comment ];
        }
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self  performSegueWithIdentifier:@"segueEditorDetailFromSGZG" sender:tableView];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([segue.identifier isEqualToString:@"segueEditorDetailFromSGZG"])
    {
    ...
    if ([segue.identifier isEqualToString:@"segueFilterProcessFromSGZJ"]) {
        [segue.destinationViewController setValue:@"E" forKey:@"role"];
    }
}
